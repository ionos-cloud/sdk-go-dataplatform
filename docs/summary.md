# Table of contents

* [Introduction](README.md)
* [Changelog](CHANGELOG.md)

## API

* [DataPlatformClusterApi](api/DataPlatformClusterApi.md)
* [DataPlatformMetaDataApi](api/DataPlatformMetaDataApi.md)
* [DataPlatformNodePoolApi](api/DataPlatformNodePoolApi.md)

## Models

* [AvailabilityZone](models/AvailabilityZone.md)
* [Cluster](models/Cluster.md)
* [ClusterListResponseData](models/ClusterListResponseData.md)
* [ClusterResponseData](models/ClusterResponseData.md)
* [CreateClusterProperties](models/CreateClusterProperties.md)
* [CreateClusterRequest](models/CreateClusterRequest.md)
* [CreateNodePoolProperties](models/CreateNodePoolProperties.md)
* [CreateNodePoolRequest](models/CreateNodePoolRequest.md)
* [ErrorMessage](models/ErrorMessage.md)
* [ErrorResponse](models/ErrorResponse.md)
* [Lan](models/Lan.md)
* [MaintenanceWindow](models/MaintenanceWindow.md)
* [Metadata](models/Metadata.md)
* [NodePool](models/NodePool.md)
* [NodePoolListResponseData](models/NodePoolListResponseData.md)
* [NodePoolResponseData](models/NodePoolResponseData.md)
* [PatchClusterProperties](models/PatchClusterProperties.md)
* [PatchClusterRequest](models/PatchClusterRequest.md)
* [PatchNodePoolProperties](models/PatchNodePoolProperties.md)
* [PatchNodePoolRequest](models/PatchNodePoolRequest.md)
* [Route](models/Route.md)
* [StorageType](models/StorageType.md)
* [VersionsGet200Response](models/VersionsGet200Response.md)
